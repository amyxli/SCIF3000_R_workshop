...
beaches2_cleaned <- beaches2 %>%
  select(MISSING_BIT(MISSING_BIT))
...
beaches2_cleaned <- beaches2_cleaned %>%
  arrange(month, day, year, site) # fix this!
...
beaches2_cleaned <- MISSING_BIT %>%
  filter(!(MISSING_BIT == MISSING_BIT & MISSING_BIT == MISSING_BIT))
...
beaches2_cleaned <- MISSING_BIT %>%
  filter(MISSING_BIT | MISSING_BIT)
...
beach_means <- beaches2_cleaned %>%
  group_by(MISSING_BIT) %>%
  summarise(
    enterococci = mean(MISSING_BIT)
  ) %>%
  ungroup()

# What do you notice here?

# We can use the skim() function from skimr to see if there is anything that
...
beach_means <- beaches2_cleaned %>%
  group_by(MISSING_BIT) %>%
  summarise(
    MISSING_BIT = mean(MISSING_BIT, na.rm = TRUE)
  ) %>%
  MISSING_BIT
...
beaches2_cleaned %>%
  ...
  arrange(desc(MISSING_BIT)) %>% # notice: what does desc() do here? what if we omit it?
  MISSING_BIT
...
beaches2_cleaned <- beaches2_cleaned %>%
  MISSING_BIT(MISSING_BIT = ifelse(MISSING_BIT))
...
beaches2_cleaned <- beaches2_cleaned %>%
  MISSING_BIT(
    MISSING_BIT = ifelse(MISSING_BIT, "A",
                    ifelse(MISSING_BIT, "B",
                           ifelse(MISSING_BIT, "C",
                                  ifelse(MISSING_BIT, "D",
                                         MISSING_BIT))))
  )
...
beach_safety <- beaches2_cleaned %>%
  group_by(MISSING_BIT, MISSING_BIT, month, month_name) %>% # monthly means
  MISSING_BIT %>%
  ungroup()
...
# You are done with all the exercises!
